...
public abstract class DAOSQLImp<T> {
    ...
    public void deleteRecord(String id) throws SQLException {
        try (PreparedStatement stmt = SQLDataSource.getStatement("DELETE FROM " + table + " WHERE " + columns[0] + " = ?")) {
            stmt.setString(1, id);
            stmt.execute();
    ...
    public void modifyRecord(T obj) throws SQLException {
        ...
        try (PreparedStatement stmt = SQLDataSource.getStatement("UPDATE TABLE " + table + " SET " + sb.toString() + " WHERE " + columns[0] + " = ?")) {
            for (int i = 0; i < data.length; ++i) {
                stmt.setObject(i + 1, data[i], columnTypes[i]);
            }
